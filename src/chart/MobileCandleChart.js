...
const CandleChart = ({ data,chartSelect,cat }) => {
...
const updateLegend = (param = {}) => {
    ...
    let isTodayHasMarker = dateStr === data.at(-1).time && markers.some(marker => marker.time === dateStr);
    window.hideWarning = function() {
        const warningDiv = document.getElementById("warningDiv");
        if (warningDiv) {
            isTodayHasMarker = false;
    ...
    if (seriesData) {
      if (chartSelect === "Line") {
        ...
        const warningMessage = hasMarker ? '<span style="margin-right: 5px;">⚠️</span>주의! 하락 위험! <br/> <span>현재 RWI가 EWI를 초과한 상태입니다</span>' : '';

        const lineInfo = `
        <div style="display: flex; align-items: center; margin-left: 10px;">
            ${router.locale==="ko" ? "가격" : "Value"}: <span style="color: ${changeColor}">${close}</span>
            
            ${isTodayHasMarker ? `<div id="warningDiv"  style="margin-left: 30px; z-index: 1001;
            background-color: red; 
            border-radius: 5px;
            padding: 0px 10px;
            color: white;
            font-size: 0.8rem;
            pointer-events: auto;   // 이 부분을 수정하여 클릭이 가능하도록 했습니다.
    ...
    `;
      ...
      const changePercent = changeValue ? changeValue.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 }) : '-';
  ...
  const warningMessage = hasMarker ? '<span style="margin-right: 5px;">⚠️</span>주의! 하락 위험! <span>현재 RWI가 EWI를 초과한 상태입니다</span>' : '';
  
  const ohlcInfo = `
  <div style="display: flex; align-items: center; margin-left: 10px; flex-wrap: wrap;">
  <div style="flex-wrap: wrap; min-width: 300px; display: flex; align-items: center; font-size: 0.7rem;"> 
      ...
      ${router.locale==="ko" ? "종가" : "Close"}: <span style="margin-right:6px; color: ${changeColor}">${close}</span> 
      ${router.locale==="ko" ? "가격변화" : "Change"}: <span style=" color: ${changeColor}">${changePercent}%</span>
      ${hasMarker ? `<div style="margin-left: 3px; margin-top:5px; z-index: 1001; background-color: red; border-radius: 5px; padding: 2px 8px; color: white; font-size: 0.8rem; pointer-events: none; font-family: Arial, sans-serif; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.2); border: 1px solid rgba(0, 0, 0, 0.1); white-space: nowrap;">${warningMessage}</div>` : ''}
  </div>
</div>
  `;
        ...
        if (targetData) {
            const formatValue = (value, noDecimal = false) => {
                ...
                const options = { 
                    ...
                    maximumFractionDigits: noDecimal ? 0 : 0
                };
            ...
            toolTip.innerHTML = `
                <ul style="list-style-type: none; padding: 0; margin: 0;">
                    <li style="margin-bottom: 4px;"><strong>${router.locale==="ko" ? "시가" : "Open"}:</strong> <span>${open}${cat!=="Index" ? "￦" : ""}</span></li>
                    <li style="margin-bottom: 4px;"><strong>${router.locale==="ko" ? "고가" : "High"}:</strong> <span>${high}${cat!=="Index" ? "￦" : ""}</span></li>
                    <li style="margin-bottom: 4px;"><strong>Low:</strong> <span>${low}${cat!=="Index" ? "￦" : ""}</span></li>
                    <li style="margin-bottom: 4px;"><strong>Close:</strong> <span>${close}${cat!=="Index" ? "￦" : ""}</span></li>
...
const handleTouchStart = (event) => {
    ...
    if (targetData) {
        ...
        if (chartSelect === "Line") {
            ...
            if (lineDataItem) {
                const value = lineDataItem.close.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 });
            ...
            const open = targetData.open.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 });
            const high = targetData.high.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 });
            const low = targetData.low.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 });
            const close = targetData.close.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 });
            const changeValue = targetData.open ? ((targetData.close - targetData.open) / targetData.open) * 100 : null;
            const changePercent = changeValue ? changeValue.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 }) : '-';
...
chartInstance.current.subscribeCrosshairMove((param) => {
  ...
  toolTip.innerHTML = `
  <ul style="list-style-type: none; padding: 0; margin: 0;">
    <li style="margin-bottom: 4px;"><strong>Open:</strong> <span>${open}${cat!=="Index" ? "￦" : ""}</span></li>
    <li style="margin-bottom: 4px;"><strong>High:</strong> <span>${high}${cat!=="Index" ? "￦" : ""}</span></li>
    <li style="margin-bottom: 4px;"><strong>Low:</strong> <span>${low}${cat!=="Index" ? "￦" : ""}</span></li>
    <li style="margin-bottom: 4px;"><strong>Close:</strong> <span>${close}${cat!=="Index" ? "￦" : ""}</span></li>
    <li style="margin-bottom: 4px;"><strong>Change:</strong> <span style="color: ${changeColor}">${change}%</span></li>
   <br/>
