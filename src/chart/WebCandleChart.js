...
const updateLegend = (param = {}) => {
  ...
  const warningMessage = hasMarker ? '<span style="margin-right: 5px;">⚠️</span>주의! 하락 위험! <br/> <span>현재 RWI가 EWI를 초과한 상태입니다</span>' : '';
  let ohlcInfoStyle = "";
  let warningMarginLeft = "10px";
  if (window.innerWidth <= 600) {
      ohlcInfoStyle = "margin-bottom: 10px;";
      warningMarginLeft = "0px";
  }
  
  const ohlcInfo = `
  <div style="display: flex; align-items: center; margin-left: 10px; flex-wrap: wrap;">
      <div style="flex-wrap: wrap; min-width: 300px; ${ohlcInfoStyle}">
      ...
      ${hasMarker ? `<div style="margin-left:${warningMarginLeft}; z-index: 1001; background-color: red; border-radius: 5px; padding: 0px 10px; color: white; font-size: 0.8rem; pointer-events: none; font-family: Arial, sans-serif; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.2); border: 1px solid rgba(0, 0, 0, 0.1); white-space: nowrap;">${warningMessage}</div>` : ''}
  </div>
  `;
