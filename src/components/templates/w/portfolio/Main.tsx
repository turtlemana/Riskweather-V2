...
function Main({
  ...
  return (
    <div className="flex flex-col h-screen pb-20 mb-20 bg-white">
      ...
      <div className="flex flex-col mt-5 px-5 min-w-[420px]">
        {portfolios &&
          portfolios?.map((portfolio: any, idx: number) => (
            <div
              ...
              key={idx}
              className="cursor-pointer flex items-center border-b justify-between"
            >
      ...
      {userProfile &&
        ...
        userProfile.portfolios.length < 3 && (
          <div className="fixed  bottom-[200px] left-0 right-0 flex justify-center">
            <button
              onClick={() => {
                dispatch({ type: "SET_ADD_OPEN", payload: true });
              }}
              className="w-1/4 mx-8 text-blue-500 font-bold bg-blue-50 rounded-lg py-2 px-4"
            >
              +포트폴리오 추가
            </button>
          </div>
